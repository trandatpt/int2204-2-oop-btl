...
public class ServerEntityPositionUpdateHandle implements PacketHandler<PacketPlayOutEntityPosition, CServerConnection> {
	@Override
	public void handle(PacketPlayOutEntityPosition packet, CServerConnection context) {
		...
		if (!world.hasEntity(entityId)) {
			System.err.println("[ENTITY POS] Entity with ID " + entityId + " does not exist in the client-bound world, ignoring.");
			return;
		}
		
		world.getEntityById(entityId).updatePositionFrom(packet);
	}
}
